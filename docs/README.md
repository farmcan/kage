# KAGE Docs

This directory keeps current product, growth, release, and future architecture documents.

## Current

- [Product strategy](product-strategy-2026-05-28.md)
- [Architecture review](architecture-review-2026-05-28.md)
- [Desktop release plan](desktop-release-plan-2026-05-29.md)
- [Growth plan](growth-plan-2026-05-28.md)
- [GitHub presence guide](github-presence-guide.md)
- [Five-round user churn audit](user-churn-audit-2026-05-31.md)
- [Release and launch checklist](release-launch-checklist.md)
- [v0.1.9 release notes](release-notes/v0.1.9.md)
- [v0.1.8 release notes](release-notes/v0.1.8.md)
- [v0.1.7 release notes](release-notes/v0.1.7.md)
- [v0.1.6 release notes](release-notes/v0.1.6.md)
- [v0.1.5 release notes](release-notes/v0.1.5.md)
- [v0.1.4 release notes](release-notes/v0.1.4.md)
- [v0.1.3 release notes](release-notes/v0.1.3.md)
- [v0.1.2 release notes](release-notes/v0.1.2.md)
- [v0.1.1 release notes](release-notes/v0.1.1.md)
- [v0.1.0 release notes draft](release-notes/v0.1.0.md)

## Future Direction

- [Mobile multi-device sessions plan](mobile-multi-device-sessions-plan-2026-05-20.md)

Older issue-fix plans and completed implementation checklists were removed once their work shipped or was superseded by the current product strategy. Git history remains the archive for those documents.
